...
cuda_supported_archs="6.0;6.1"
sed -i.orig -E "s/set\(CUDA_SUPPORTED_ARCHS \"([^\"]*)\"\)/set(CUDA_SUPPORTED_ARCHS \"${cuda_supported_archs};\1\")/g" CMakeLists.txt

echo " - Updating Dockerfile"
torch_cuda_arch_list="6.0 6.1 7.0 7.5 8.0 8.6 8.9 9.0+PTX"
sed -i.orig "s/ARG torch_cuda_arch_list='[^']*'/ARG torch_cuda_arch_list='${torch_cuda_arch_list}'/" docker/Dockerfile
cat <<EOF
